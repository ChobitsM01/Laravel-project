...
class FrontendController extends Controller
{
    ...
    {
        $mail = $request->email;
        $checkMail = User::where('email_address', $mail)->first();
        $auth = Auth()->user()->email_address;
        if ($checkMail != $auth) {
            return redirect()->back()->with('error', 'Mail không đúng');
        }
        $query = order::where('email', $mail)->count();
        if ($query > 5) {
            ...
            return redirect()->back()->with('success', 'Bạn đã nhận được mã giảm giá!.');
        }else{
            return redirect()->back()->with('error', 'Bạn chưa đủ điều kiện để nhận được mã giảm giá!.');
        }
    }
}
